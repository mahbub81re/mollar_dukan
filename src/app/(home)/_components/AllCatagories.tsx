...
export default  function AllCatagories() {
  ...
  return (
    <div className=' w-full h-auto pb-16 bg-local ' style={{backgroundImage: `url("/catbg.png")`}}>
     <SecTitle dot_color="bg-[#ff3131]" title="Catagories" text_color="text-[#ff3131]"/>
      {/* cata list */}
      <div className="flex flex-row justify-center ">
        <div className={`grid grid-cols-1 md:grid-cols-2 text-[24px]  md:space-x-6 max-md:space-y-16 ` }>
          {catagories.map((category:any)=>{
                 return(
                 <Link href={"/products/"+category._id} key={category._id}>
