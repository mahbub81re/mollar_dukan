    ...
    try{
        if(token?.email==="admin@gmail.com"){
            ...
            if(res){
                ...
                return NextResponse.json({success:false,status:402,message:"Did not Found category"})
            }
        }else{
            return NextResponse.json({success:false,status:403,message:"Not Authorized"})
        }
    }catch(err){
        return NextResponse.json({success:false,status:402,message:"Something's wrong"})
    }
}